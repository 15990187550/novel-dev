...
export const listABExperiments = () => api.get('/ab-tests').then(r => r.data);
export const listDecisionsByExperiment = (id) => api.get(`/ab-decisions/by-experiment/${id}`).then(r => r.data);
export const getRecentABDecisions = (windowMinutes = 60) =>
  api.get('/ab-decisions/recent', { params: { window_minutes: windowMinutes } }).then(r => r.data);
export const fetchJudgePromptVersions = () => api.get('/judge-prompt-versions').then(r => r.data);
export const createJudgePromptVersion = (payload) => api.post('/judge-prompt-versions', payload).then(r => r.data);
export const activateJudgePromptVersion = (pvId) => api.post(`/judge-prompt-versions/${pvId}/activate`).then(r => r.data);
export const fetchJudgeCallStats = (params = {}) => api.get('/judge-call-stats', { params }).then(r => r.data);
export const triggerJudgeSweeperTick = () => api.post('/judge-sweeper/tick').then(r => r.data);
