    ...
    async def check_writer_beat(
        ...
        *,
        ...
        prompt = (
            ...
            "3. 不得新增章节计划外的重大剧情事实、核心角色、关键物件、关键证据、独立威胁实体或背景因果。\n"
            "4. 不得改变已完成文本与当前 beat 的事件顺序。\n\n"
            "## 允许的最小桥接细节\n"
            "为兑现当前 beat 的目标、阻力、选择、代价或停点，允许出现无名动作、短台词、环境声响、灯火、脚步、视线、"
            "已有实体的反应等轻量细节；这些细节不能成为新主线线索，也不能替代后续 beat 的核心事件。"
            "桥接不得新增命名实体、新能力规则、新证据链、新阵营身份或新的长期因果解释。\n\n"
            f"### beat_index\n{beat_index}\n\n"
    ...
    async def check_editor_beat(
        ...
        *,
        ...
        prompt = (
            ...
            "只返回符合 ChapterStructureGuardResult schema 的 JSON。\n\n"
            "## 判定规则\n"
            "1. 润色后不得新增章节计划外的重大剧情事实、核心角色、关键物件、关键证据、独立威胁实体或背景因果。\n"
            "2. 润色后不得改变事件先后顺序。\n"
            "3. 润色后不得提前写后续 beat 的核心事件。\n"
            "4. 允许删减冗余、改善文风、压缩 AI 腔表达。\n\n"
            "5. 允许用无名动作、停顿、视线、环境声响或已有实体反应增强当前 beat 的读感，但不得形成新线索、新事件、"
            "新命名实体、新能力规则或新因果解释。\n\n"
            "6. 当前 beat 是章节计划 beats[beat_index]；当前 beat 摘要、写作卡或 required_payoffs 中明确要求的追兵、证物、选择、停点，"
            "不算后续 beat 泄露。只有 beats[beat_index+1:] 的核心事件才算提前。\n\n"
            f"### beat_index\n{beat_index}\n\n"
            f"### 章节计划\n{json.dumps(_to_jsonable(chapter_plan), ensure_ascii=False)}\n\n"
            f"### 润色前\n{source_text[:4000]}\n\n"
            f"### 润色后\n{polished_text[:4000]}"
