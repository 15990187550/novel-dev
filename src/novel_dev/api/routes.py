...
async def list_approved_documents(novel_id: str, doc_type: Optional[str] = None, session: AsyncSession = Depends(get_session)):
    ...
    embedding_service = EmbeddingService(session, embedder)
    svc = ExtractionService(session, embedding_service)
    docs = await svc.list_approved_documents(novel_id)
    if doc_type:
...
async def get_approved_document(novel_id: str, document_id: str, session: AsyncSession = Depends(get_session)):
    embedder = llm_factory.get_embedder()
    embedding_service = EmbeddingService(session, embedder)
    svc = ExtractionService(session, embedding_service)
...
async def list_document_versions_by_type(novel_id: str, doc_type: str, session: AsyncSession = Depends(get_session)):
    repo = DocumentRepository(session)
    docs = await repo.list_versions(novel_id, doc_type)
    return {
        "items": [
            {
                "id": d.id,
...
async def list_document_versions(novel_id: str, document_id: str, session: AsyncSession = Depends(get_session)):
    embedder = llm_factory.get_embedder()
    embedding_service = EmbeddingService(session, embedder)
    svc = ExtractionService(session, embedding_service)
    # First verify the document exists and belongs to the novel
    doc = await svc.get_approved_document(novel_id, document_id)
    if doc is None:
        raise HTTPException(status_code=404, detail="Document not found")
    versions = await svc.list_document_versions(novel_id, document_id)
    return {
        "items": [
            {
                "id": d.id,
