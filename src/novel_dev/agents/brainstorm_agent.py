...
class BrainstormAgent:
    ...
    async def brainstorm(self, novel_id: str) -> SynopsisData:
        ...
        synopsis_data = await self._generate_and_refine(combined)
    ...
    async def _generate_and_refine(self, combined_text: str) -> SynopsisData:
        """生成→打分→修订的 self-review 循环。synopsis 是整部小说的根,一次生成容易糟。"""
        synopsis = await self._generate_synopsis(combined_text)
        for attempt in range(self.MAX_REVISE_ATTEMPTS):
            score = await self._score_synopsis(synopsis)
            if self._is_acceptable(score):
                return synopsis
            synopsis = await self._revise_synopsis(synopsis, score, combined_text)
        # 达到最大修订次数仍不达标,返回最后一版(避免阻塞流程,让下游 VolumePlanner 兜底)
        return synopsis
    ...
    async def _score_synopsis(self, synopsis: SynopsisData) -> SynopsisScoreResult:
        from novel_dev.llm import llm_factory
        client = llm_factory.get("BrainstormAgent", task="score_synopsis")
        prompt = (
        ...
        return SynopsisScoreResult.model_validate_json(response.text)
    ...
    async def _revise_synopsis(
        ...
        from novel_dev.llm import llm_factory
        client = llm_factory.get("BrainstormAgent", task="revise_synopsis")
