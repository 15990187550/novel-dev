...
BRAINSTORM_PROMPT = (
    ...
    "## 结构转折覆盖要求(硬性)\n"
    "milestones 中至少需要命中以下 4 个不同类目的关键词,每幕至少覆盖一个类目,"
    "否则质量门禁会判定结构分不及格:\n"
    "- 失去/覆灭类(loss_or_fall):使用『覆灭、失去、沦为、废、重伤、败亡、被逐、陷害』等关键词。\n"
    "- 结盟/背叛类(alliance_or_betrayal):使用『结盟、联手、联盟、背叛、破裂、拔剑指向、护他、押上身份』。\n"
    "- 线索/揭秘类(clue_or_reveal):使用『发现、得知、确认、揭开、揭露、真相、证据、线索、浮出水面』。\n"
    "- 威胁升级类(threat_escalation):使用『刺杀、围杀、追捕、追杀、陷阱、被迫逃、逃入、逃亡、拿下』。\n"
    "- 力量变化类(power_shift):使用『能力变化、暴露实力、实力变化、获得、关键资源、反噬、失控』。\n"
    "- 身份/世界改变类(identity_or_world_change):使用『身世、身份、宿主、封印、异常物、秩序根基、世界边界、崩塌』。\n"
    "- 选择/牺牲类(choice_or_sacrifice):使用『必须选择、选择、放弃、代价、以凡人之躯、自残、牺牲』。\n"
    "每个 milestone 的 act / summary / climax_event 文本中至少出现 1 个目标类目的具体关键词,"
    "且至少 4 个不同 milestone 各覆盖一个不同类目。\n\n"
    "## 金手指(主角外挂)识别\n"
    "如果设定资料中存在主角的特殊能力、系统机制、关键资源、身份优势或觉醒等外挂机制,"
    "必须在 character_arcs 中为该主角的弧线额外补充 cheat 字段,"
    "并按以下结构描述——这是后续 VolumePlanner 标记『金手指激活章』与"
    "ContextAgent 注入激活规则的依据:\n"
    "- cheat.ability: 一句话描述资料明示的核心机制,"
    "只写资料里明示的能力,不要脑补。\n"
    "- cheat.activation_rules: 字符串数组,逐条列出资料明示的触发/激活条件,"
    "资料未明示时填空数组。\n"
    ...
    "- character_arcs: 数组(必填,不能为空),至少包含主角 + 关键对手/盟友 2 条弧线,"
    "前 2 条弧线的 key_turning_points 必须 ≥3。每项只包含 name / arc_summary / key_turning_points 三个字段。"
...
VOLUME_PLANNER_PROMPT = (
    ...
    "1.2 力量层级不能跨章跃迁——一个章节只能推进主角当前境界内的前/中/后/圆满小阶,或相邻境界内的 1 个小阶,"
    "不得在单章或数章内一次性跨越 2 个及以上大境界(炼气→筑基→金丹→元婴→化神等)。"
    "如不确定本卷应停在哪个境界,默认停在主角起点境界的前/中阶,留出大量过渡章。\n"
    "2. 高阶敌人、终局真相、后续世界/体系若未在本卷允许范围内，只能写成伏笔、残痕、传闻、代理人或异常现象。\n"
    "3. 缺少设定依据时不得硬编关键事实，应保守降级为待确认线索。\n"
    "4. 不得重新引入用户已删除或未批准的旧设定。\n\n"
    "5. 力量层级、技能层级、组织层级等专有层级名称必须逐字来自总纲、当前设定或 ActiveConstraintContext；"
    "不得按通用类型套路自造未提供的层级编号或等级名。\n\n"
    "## 输出规模限制\n"
    "{scale_rule}"
...
判定标准:
- 重要: 能力阶跃、位置大跨度、关系反转、状态变化(生/死)、新身份获得
...
2. 身份漂移: 同一角色身份称谓变化(如 上级→下属)
3. 状态阶跃: 实体状态变化无伏笔/无交代(如 突然获得高阶能力)
