...
- `处理`：打开详情抽屉。
- `回填对话`：把建议卡内容整理成反馈 prompt，写入现有大纲对话输入框。
...
`回填对话` 不调用后端，沿用现有 `OutlineConversation.setDraft()`。前端基于卡片生成结构化 prompt：
...
如果 payload 中有关键字段，追加到“需要补充/确认的设定字段”中。用户仍可编辑后再提交。
...
2. 前端按状态拆分待处理列表和历史列表。
3. 用户点 `回填对话`，前端生成 prompt 并写入现有输入框。
...
- 接收 `回填对话` 事件后写入现有 conversation draft。
