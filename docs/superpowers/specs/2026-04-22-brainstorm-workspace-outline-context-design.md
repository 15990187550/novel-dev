...
6. **流程复用而非运行时直调**
   `superpowers:brainstorming` 复用的是其方法论与流程约束，而不是在产品运行时直接调用 skill 文件。

### 2.1 `superpowers:brainstorming` 的复用方式

本设计选择将 `superpowers:brainstorming` 作为产品内脑爆协议的来源，而不是运行时依赖：
...
- **不采用：**
  - 在后端服务中直接读取并执行 skill 文件
  - 将 skill 文件本身视为稳定业务 API
  - 将产品流程绑定到外部 agent 是否真实加载该 skill

原因：

- 产品能力需要稳定、可测试、可版本化
- `BrainstormWorkspace` 需要成为正式落库与提交的权威业务载体
- skill 更适合作为编排规范，而不是运行时依赖
